 ...
 */
public class BatchGDFMLearner<I extends FMInstance> extends GDFMLearner<I> {

    private static final Logger LOG = Logger.getLogger(BatchGDFMLearner.class.getName());
     ...
     */
    public BatchGDFMLearner(double alpha, int numIter, FMError<I> error) {
        super(alpha, error);
    ...
    public void learn(FM<I> fm, FMData<I> train, FMData<I> test) {
        ...
        for (int t = 1; t <= numIter; t++) {
            ...
            fm.setB(fm.getB() - alpha * gradB.doubleValue() / train.numInstances());
            fm.getW().assign(gradW, (r, s) -> r - alpha * s / train.numInstances());
            fm.getM().assign(gradM, (r, s) -> r - alpha * s / train.numInstances());
